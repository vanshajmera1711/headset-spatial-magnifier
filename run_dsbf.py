import os
import json
import torch
import torchaudio
import soundfile as sf

# ==========================================
# 1. BULLETPROOF SOUNDFILE PATCHES 
# ==========================================
def soundfile_load_patch(filepath, frame_offset=0, num_frames=-1, convert=True, channels_first=True):
    data, samplerate = sf.read(filepath, start=frame_offset, frames=num_frames, dtype='float32', always_2d=True)
    tensor = torch.from_numpy(data)
    if channels_first:
        tensor = tensor.t()
    return tensor, samplerate

def soundfile_save_patch(filepath, src, sample_rate, channels_first=True, bits_per_sample=16, format=None, encoding=None):
    if channels_first and src.ndim > 1:
        data = src.t().numpy()
    else:
        data = src.numpy()
    sf.write(filepath, data, sample_rate, subtype=f'PCM_{bits_per_sample}')

# REMOVED: torchaudio.load / save overrides to prevent infinite recursion loop

# ==========================================
# 2. EXACT SI-SNR METRIC COMPUTATION
# ==========================================
def compute_si_snr(estimate, reference, epsilon=1e-8):
    estimate = estimate - estimate.mean()
    reference = reference - reference.mean()

    min_len = min(estimate.shape[-1], reference.shape[-1])
    estimate = estimate[..., :min_len]
    reference = reference[..., :min_len]

    reference_pow = reference.pow(2).sum(dim=-1, keepdim=True)
    mix_pow = (estimate * reference).sum(dim=-1, keepdim=True)
    scale = mix_pow / (reference_pow + epsilon)

    target = scale * reference
    error = estimate - target

    target_pow = target.pow(2).sum(dim=-1)
    error_pow = error.pow(2).sum(dim=-1)

    si_snr_val = 10 * torch.log10(target_pow / (error_pow + epsilon))
    return si_snr_val.mean().item()


# 3. CONFIGURATION & DIRECTORIES (UPDATED)
# ==========================================
# Points directly to your generated 1000-sample dataset repository paths
INPUT_FOLDER  = r"C:\projects\headset-spatial-magnifier\data\generated_dataset"
OUTPUT_FOLDER = r"C:\projects\headset-spatial-magnifier\data\dsbf_output"
METRICS_FOLDER = r"C:\projects\headset-spatial-magnifier\data"

SAMPLE_RATE       = 16000
REFERENCE_CHANNEL = 1   # Aligned to our master evaluation channel (left_front)
N_FFT = 1024
N_HOP = 256

os.makedirs(OUTPUT_FOLDER, exist_ok=True)
os.makedirs(METRICS_FOLDER, exist_ok=True)

stft  = torchaudio.transforms.Spectrogram(n_fft=N_FFT, hop_length=N_HOP, power=None)
istft = torchaudio.transforms.InverseSpectrogram(n_fft=N_FFT, hop_length=N_HOP)

# ==========================================
# 4. CLUSTER-WEIGHTED COMBINATION (DSBF BASELINE)
# ==========================================
def delay_and_sum_beamform(stft_transform, istft_transform, waveform_mix, length,
                           clusters=((0, 1, 2), (3, 4, 5))):
    """
    Cluster-aware Beamforming Baseline.
    Averages within each mic cluster independently, then combines
    cluster outputs by magnitude-weighted selection in the STFT domain.
    """
    stft_mix = stft_transform(waveform_mix)  # (C, F, frames)
    num_channels = waveform_mix.shape[0]

    cluster_stfts = []
    for ch_indices in clusters:
        valid_indices = [idx for idx in ch_indices if idx < num_channels]
        if not valid_indices:
            continue
            
        stft_cluster = stft_mix[valid_indices].mean(dim=0)  # (F, frames)
        cluster_stfts.append(stft_cluster)

    if not cluster_stfts:
        return stft_mix[0].unsqueeze(0)

    # Magnitude-weighted combination across clusters
    mags = torch.stack([s.abs() for s in cluster_stfts], dim=0)   # (N_clusters, F, frames)
    weights = mags / (mags.sum(dim=0, keepdim=True) + 1e-8)
    stft_ds = sum(w * s for w, s in zip(weights.unbind(0), cluster_stfts))

    waveform_ds = istft_transform(stft_ds, length=length)
    return waveform_ds.unsqueeze(0).to(torch.float32)

# ==========================================
# 5. FIXED PROCESSING LOOP FOR TARGET_GT STEMS
# ==========================================
all_files = os.listdir(INPUT_FOLDER)

# Gather only the true mixture files based on your directory structure
mixture_files = [f for f in all_files if f.endswith("_mix.wav") or (f.endswith("_mix") and not f.endswith(".json"))]

if not mixture_files:
    mixture_files = [f for f in all_files if f.endswith("_mix")]

scenes_processed  = 0
total_sisnr_in    = 0.0
total_sisnr_out   = 0.0
metrics_log       = {}

print(f"Starting analysis on dataset mixtures inside: {INPUT_FOLDER}\n")

for filename in mixture_files:
    base_ext    = ".wav" if filename.endswith(".wav") else ""
    file_path   = os.path.join(INPUT_FOLDER, filename if base_ext else f"{filename}.wav")

    target_filename = filename.replace("_mix", "_target_gt")
    target_path = os.path.join(INPUT_FOLDER, target_filename if base_ext else f"{target_filename}.wav")

    if not (os.path.exists(file_path) and os.path.exists(target_path)):
        continue

    try:
        # Load audio signals using patches directly to avoid recursion error
        waveform_mix,  sr   = soundfile_load_patch(file_path)
        waveform_clean, sr_c = soundfile_load_patch(target_path)

        if waveform_mix.shape[0] < 2:
            continue  

        # Shape alignment
        min_samples    = min(waveform_mix.shape[-1], waveform_clean.shape[-1])
        waveform_mix   = waveform_mix[:, :min_samples]
        waveform_clean = waveform_clean[:, :min_samples]

        # Isolate anchor reference channel arrays
        clean_ref = waveform_clean[REFERENCE_CHANNEL:REFERENCE_CHANNEL+1]
        mix_ref   = waveform_mix[REFERENCE_CHANNEL:REFERENCE_CHANNEL+1].to(torch.float32)

        # Calculate baseline input SI-SNR using the raw unprocessed reference channel
        sisnr_in = compute_si_snr(mix_ref, clean_ref)

        # ----- Beamformer Execution (Option C: Single-Mic Baseline) -----
        # Solves the phase cancellation drop while maintaining script structure
        waveform_ds = delay_and_sum_beamform(
            stft, 
            istft, 
            waveform_mix.to(torch.float32), 
            waveform_mix.shape[-1],
            clusters=((1,), (4,))
        )

        # Calculate True Output SI-SNR
        sisnr_out  = compute_si_snr(waveform_ds, clean_ref)
        sisnr_imp  = sisnr_out - sisnr_in
        
        # Sanity check downmix metrics vs raw channel metrics
        mix_avg = waveform_mix.mean(dim=0, keepdim=True).to(torch.float32)
        sisnr_downmix = compute_si_snr(mix_avg, clean_ref)
        print(f"  Ref-Channel In: {sisnr_in:.2f} dB  |  DS out: {sisnr_out:.2f} dB  |  Raw Downmix: {sisnr_downmix:.2f} dB")

        # Save Enhanced File using patch directly
        output_path = os.path.join(OUTPUT_FOLDER, f"enhanced_{filename if base_ext else f'{filename}.wav'}")
        soundfile_save_patch(output_path, waveform_ds, SAMPLE_RATE)

        # Update metrics
        scenes_processed += 1
        total_sisnr_in   += sisnr_in
        total_sisnr_out  += sisnr_out

        metrics_log[filename] = {
            "sisnr_input":        round(sisnr_in,  2),
            "sisnr_output":       round(sisnr_out, 2),
            "sisnr_improvement": round(sisnr_imp, 2)
        }
        print(f"Processed scene {scenes_processed}: {filename} (Gain: {sisnr_imp:+.2f} dB)\n")

    except Exception as e:
        print(f"Error handling file {filename}: {str(e)}")


# ==========================================
# 6. GENERATE FINAL METRICS & CONSOLE PRINT
# ==========================================
if scenes_processed > 0:
    mean_in  = total_sisnr_in  / scenes_processed
    mean_out = total_sisnr_out / scenes_processed
    mean_imp = mean_out - mean_in
else:
    mean_in, mean_out, mean_imp = 0.0, 0.0, 0.0

metrics_json_path = os.path.join(METRICS_FOLDER, "dsbf_metrics.json")
summary_data = {
    "scenes_processed":        scenes_processed,
    "mean_sisnr_input_db":      round(mean_in,  2),
    "mean_sisnr_output_db":     round(mean_out, 2),
    "mean_sisnr_improvement_db": round(mean_imp, 2),
    "per_file_metrics":        metrics_log
}

with open(metrics_json_path, "w") as f:
    json.dump(summary_data, f, indent=4)

print("\n--- DSBF Results ---")
print(f"Scenes processed        : {scenes_processed}")
print(f"Mean SI-SNR input       : {mean_in:.2f} dB")
print(f"Mean SI-SNR output      : {mean_out:.2f} dB")
print(f"Mean SI-SNR improvement : {mean_imp:.2f} dB")
print(f"Outputs saved to        : data/dsbf_output")
print(f"Metrics saved to        : data/dsbf_metrics")