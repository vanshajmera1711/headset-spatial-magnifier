...
def create_headset_magnifier_array(centroid_x, centroid_y, height):
    ...
    return np.array([
        v_left_outer, real_left_front, real_left_back,
        real_right_back, real_right_front, v_right_outer
    ]).T
...
def load_and_rescale_audio(file_path, target_len_samples, target_fs=16000):
    ...
    # FIX 1: resample from DNS native 48 kHz → 16 kHz (was missing before)
    if fs != target_fs:
...
def simulate_single_scene(sample_idx, speech_files, noise_files, output_dir):
    ...
    # 4. Generate Random Coordinates for ALL sources — do NOT add to room yet
    def get_valid_coords():
        while True:
            coords = np.array([
                random.uniform(0.5, room_L - 0.5),
    ...
    interf_coords_list = [get_valid_coords() for _ in s_interfs]
    noise_coords      = get_valid_coords()

    # FIX 2: add each source EXACTLY ONCE, in a fixed order:
    #   index 0              → target
    #   index 1 .. N        → interferers
    #   index N+1           → noise
    ...
    # Source index bookkeeping (mirrors add_source order above)
    target_src_idx = 0
    interf_src_idxs = list(range(1, 1 + num_interferers))
    noise_src_idx   = 1 + num_interferers

    # 6. Isolate Ground-Truth Components via manual convolution
