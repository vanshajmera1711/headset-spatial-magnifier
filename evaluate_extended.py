...
def evaluate_single_sample(idx):
    ...
    try:
        ...
        # ---- PESQ (needs careful alignment -- unlike SI-SNR/STOI, PESQ's
        # internal perceptual model is sensitive to small constant time
        # shifts and level mismatches introduced by the STFT/ISTFT round
        # trip. We fix both before scoring: ----
        if HAVE_PESQ:
            try:
                pesq_score = compute_pesq_aligned(tgt_ref, enh, SAMPLE_RATE)
