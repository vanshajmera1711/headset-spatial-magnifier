...
class EnhancementExp(pl.LightningModule):
    ...
    def log_batch_detailed_audio(self, noisy_td, enhanced_td, batch_idx: Union[int, None], tag: str,
        ...
        for i in range(min(self.trainer.datamodule.batch_size, n_samples, cur_samples)):
            ...
            # --- NORMALIZATION FIX ---
            # Normalize by the max absolute value to ensure the range is [-1, 1]
            # Add a small epsilon (1e-8) to prevent division by zero for silent clips.
            noisy_sample = noisy_td[i] / (torch.max(torch.abs(noisy_td[i])) + 1e-8)
            enhanced_sample = enhanced_td[i] / (torch.max(torch.abs(enhanced_td[i])) + 1e-8)

            tensorboard.add_audio(log_noisy_name, noisy_sample, global_step=self.current_epoch,
                                  sample_rate=self.trainer.datamodule.fs)

            tensorboard.add_audio(log_enhanced_name, enhanced_sample, global_step=self.current_epoch,
                                  sample_rate=self.trainer.datamodule.fs)
